...
function groupOddEven(numbers) {
  var odds = []; // Untuk menampung nomor2 ganjil
  var evens = []; // Unutk menampung nomor2 genap

  for (var i = 0; i < numbers.length; i++) {
    if (numbers[i] % 2 == 0) {
      // Jika nomor genap, tambahkan ke variable array evens.
      evens.push(numbers[i]);
    } else {
      // Jika nomor ganjil, tambahkan ke variable array odds.
      odds.push(numbers[i]);
    }
  }

  if (odds.length > 0 && evens.length > 0) {
    // Jika odds & evens tidak kosong.
    return 'ODDS: ' + odds.join(', ') + ' EVENS: ' + evens.join(', ');
  } else if (odds.length > 0 && evens.length === 0) {
    // Jika odds tidak kosong tapi evens kosong.
    return 'ODDS: ' + odds.join(', ');
  } else if (odds.length === 0 && evens.length > 0) {
    // Jika evens tidak kosong tapi odds kosong.
    return 'EVENS: ' + evens.join(', ');
  } else {
    // Jika evens & odds kosong.
    return '';
  }
}
